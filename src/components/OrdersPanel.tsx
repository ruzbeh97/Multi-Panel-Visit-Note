    ...
    <aside className="scrollbar-thin sticky top-0 ml-4 flex h-full w-[484px] shrink-0 flex-col overflow-y-auto border-l border-[#e6e6e6] bg-white px-4 pt-5">
      ...
      <div className="flex w-full flex-col gap-2 pb-10 pt-4">
        {PAST_ORDERS.map((order) => (
          <div key={order.title} className="flex w-full flex-col px-2 py-2">
            <div className="flex h-7 w-full items-center gap-2">
              <Icon name={order.icon} size={16} className={`shrink-0 ${ICON_TONES[order.tone]}`} />
              <p className="min-w-0 flex-1 truncate font-body text-[14px] font-bold leading-[22px] text-[#1a1a1a]">
                {order.title}
              </p>
              <button
                type="button"
                className="flex size-7 shrink-0 items-center justify-center rounded-full hover:bg-black/5"
                aria-label={`Open the ${order.title} order as a PDF`}
              >
                <Icon name="picture_as_pdf" size={20} className="text-[#1a1a1a]" />
              </button>
            </div>
